...
func NewTestDBW(dir string) (*DBW, error) {
	db, err := sql.Open("sqlite", filepath.Join(dir, "build", "data", "test.db"))
	if err != nil {
		return nil, fmt.Errorf("sql.Open %w", err)
	}
...
func InitSchema(ctx context.Context, dbw *DBW) error {
	_, err := dbw.DB.Exec(`PRAGMA foreign_keys = ON;`)
	if err != nil {
		return fmt.Errorf("db.Exec \"PRAGMA foreign_keys = ON;\" %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("qtx.CreateUsersTable %w", err)
	}

	err = qtx.CreateDatasetDegreesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetDegreesTable %w", err)
	}

	err = qtx.CreateDatasetIndustriesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetIndustriesTable %w", err)
	}

	err = qtx.CreateDatasetLocationsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetLocationsTable %w", err)
	}

	err = qtx.CreateDatasetSkillsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetSkillsTable %w", err)
	}
	err = qtx.CreateDatasetSpecialtiesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetSpecialtiesTable %w", err)
	}

	err = qtx.CreateDatasetStudyFieldsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateDatasetStudyFieldsTable %w", err)
	}

	err = qtx.CreateOrganizationsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateOrganizationsTable %w", err)
	}
	_, err = tx.Exec(`CREATE INDEX IF NOT EXISTS index_organization_name ON organizations("name", universal_name);`)
	if err != nil {
		return fmt.Errorf("tx.Exec CREATE organizations indexes %w", err)
	}

	err = qtx.CreateOrganizationLocationsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateOrganizationLocationsTable %w", err)
	}

	err = qtx.CreateOrganizationSpecialtiesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateOrganizationSpecialtiesTable %w", err)
	}

	err = qtx.CreateOrganizationIndustriesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateOrganizationIndustriesTable %w", err)
	}

	err = qtx.CreatePersonsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreatePersonsTable %w", err)
	}
	_, err = tx.Exec(`
	...
	`)
	if err != nil {
		return fmt.Errorf("tx.Exec CREATE persons indexes %w", err)
	}

	err = qtx.CreatePersonSkillsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreatePersonSkillsTable %w", err)
	}

	err = qtx.CreateNetworksTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateNetworksTable %w", err)
	}

	err = qtx.CreateConnectionsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateConnectionsTable %w", err)
	}

	err = qtx.CreateExperiencesTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateExperiencesTable %w", err)
	}
	_, err = tx.Exec(`
	...
	`)
	if err != nil {
		return fmt.Errorf("tx.Exec CREATE experiences indexes %w", err)
	}

	err = qtx.CreateEducationsTable(ctx)
	if err != nil {
		return fmt.Errorf("qtx.CreateEducationsTable %w", err)
	}
	_, err = tx.Exec(`
	...
	`)
	if err != nil {
		return fmt.Errorf("tx.Exec CREATE educations indexes %w", err)
	}
